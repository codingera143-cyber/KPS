...
// 2. Disable Key Shortcuts (Windows & Mac)
document.onkeydown = function(e) {
    // Disable F12
    if (e.keyCode == 123) return false;

    // Disable Ctrl+U or Cmd+U (View Source)
    if ((e.ctrlKey || e.metaKey) && e.keyCode == 85) return false;

    // Disable Ctrl+Shift+I / Cmd+Opt+I (Inspect)
    if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.keyCode == 73) return false;

    // Disable Ctrl+Shift+J / Cmd+Opt+J (Console)
    if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.keyCode == 74) return false;

    // Disable Ctrl+Shift+C / Cmd+Opt+C (Element Selector)
    if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.keyCode == 67) return false;

    // Disable Ctrl+S / Cmd+S (Save Page)
    if ((e.ctrlKey || e.metaKey) && e.keyCode == 83) return false;
};

// 3. Prevent Text/Image Selection (Optional but Professional)
// Isse koi aapka data copy-paste nahi kar payega
...
async function updateSmartContext() {
    ...
    const smartBar = document.getElementById("smart-context-box");
    ...
    if (day === 0) {
        msgElement.innerText = "Today is Sunday. Enjoy Your Day! 🏠";
        if(iconElement) iconElement.innerText = "🕶️";
        return; // Sunday mil gaya toh aage sheet check karne ki zarurat nahi
    }

    try {
        // 2. Fetch from your existing Sheet URL
        const response = await fetch(STATUS_URL); 
        const csvData = await response.text();
        const rows = csvData.split('\n').map(row => row.split(','));
        ...
        // 3. SPECIAL HOLIDAY Check (A2 and B2)
        if (statusValue.includes('closed')) {
            msgElement.innerText = `Today is Holiday On Account of ${holidayName || 'Special Occasion'}.`;
            if(iconElement) iconElement.innerText = "🚩";
        } 
        else {
            // 4. NORMAL SCHOOL TIME LOGIC (Agar Sunday/Holiday nahi hai)
            let message = "";
            let icon = "";
        ...
        msgElement.innerText = "Welcome to K.P.S. Digital Portal.";
    }
}
